...
5. Start-of-tick review gate: before selecting implementation work, review the whole project for process/design/testing/security/maintainability gaps. Check recent commits, open issues, closed issues from the previous tick, and current code structure. Do NOT create new GitHub issues during scheduled ticks by default. The scheduled job's primary role is to process existing open `project:manager` issues. If the issue board has no open implementation-ready `project:manager` issues, record the review result in `PROGRESS.md` and finish without source changes. If a serious concrete blocker/risk is discovered that would make the repository unsafe or unverifiable if left untracked, the job may create or update exactly one `status:pending` proposal/triage issue with provenance, but it must NOT implement that newly created issue in the same tick. Avoid duplicate issues; search/list existing issues first. Any proposal must be actionable, scoped, and labeled with `project:manager`, an appropriate `type:*`, `status:pending`, and a `priority:*` label.

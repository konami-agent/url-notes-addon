...
## Implemented in v0.2 — Floating note

v0.2 implements an optional activeTab-injected floating note panel on the current page.
...
- Implemented in v0.2 after owner mandate: permission/security review is encoded in validation, keeping the feature local-only with no broad host permissions and no static content scripts.
- v0.2 acceptance criteria: user can open a floating panel on the active HTTP(S) page, edit the same normalized URL note, and close it without changing page layout permanently.

...
- Popup UI plus an explicit optional floating note panel for the current active page.
...
URL notes, domain notes, and the floating note action are available on normal http:// and https:// pages. The extension intentionally does not create note keys for about:, data:, file:, extension, and other non-web tabs, and it also rejects credential-bearing web URLs such as `https://user@example.com/`.
...
When the current tab is unsupported, the popup reports that URL notes are unavailable and the editing, floating note, import, export, and search controls are disabled. Existing notes remain in local browser storage and can still be used from supported web pages.
...
Use **Import JSON** to merge a backup into local extension storage. The `notes` and `domainNotes` fields must be JSON object maps, not arrays; array-shaped maps and other malformed files are rejected before saving, so existing notes are not partially corrupted. Imported URL notes are normalized with the same URL-key rules as saved notes. Imported domain notes must be canonical DNS-style hosts or canonical IPv4 literals, and they are normalized to lowercase host keys. Leading-zero, shortened, or parser-coerced IPv4-like domain keys are rejected before saving instead of being silently converted to a different host. If an imported URL or domain normalizes to an existing note key, the imported note overwrites the existing note for that key. Blank imported notes are ignored. Invalid files and conflicting duplicate normalized URL or domain keys are rejected before saving. Duplicate raw keys with identical note text may be collapsed and imported once.
